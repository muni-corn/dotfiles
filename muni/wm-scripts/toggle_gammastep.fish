...
if systemctl --user --quiet is-active gammastep
    set nid (dunstify -p -u low "Turning $feature_name off" "Turn $feature_name on before bed to help you sleep better.")
    systemctl --user stop gammastep
    and dunstify -u low -t 5000 -r $nid "$feature_name is now off" "Turn $feature_name on before bed to help you sleep better."
else
    systemctl --user start gammastep
    and dunstify -u low -t 5000 "$feature_name is now on" "$feature_name filters blue light to help you sleep when it's late."
end
