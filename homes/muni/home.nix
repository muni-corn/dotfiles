...
let
  ...
  colors = config.muse.theme.arpeggio.generatedPalette;
  ...
  lockCmd = "$HOME/.config/sway/scripts/lock.fish --bg-color ${colors.black_0} --fg-color ${white} --primary-color ${colors.purple} --warning-color ffaa00 --error-color ff0000";
in
{
  imports = [
    ./muse
  ...
  wayland.windowManager.sway = import ./sway/mod.nix {
    inherit colors config lib pkgs bemenuArgs lockCmd;
  };
