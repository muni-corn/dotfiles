...
let
  ...
  mkColorBaseOption = name: lib.mkOption {
    type = types.strMatching "^[A-Fa-f0-9]{6}$";
    description = "The ${name} color.";
    visible = false;
  };

  optionsFromNames = names: builtins.listToAttrs (map (name: { inherit name; value = mkColorBaseOption name; }) names);
...
in
{
  ...
  paletteType =
    ...
    in
    types.submodule {
      options = (optionsFromNames baseNames) // {
        swatch = lib.mkOption {
          type = swatchType;
